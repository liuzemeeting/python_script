...
from apps.utils.db import CRUDMixin, model_to_dict
...
def json_response(code=ResCode.RES_OK, msg=u"", next="", error="", data=None, select_cols=[]):
    """
    hszhang 后台查询数据统一返回给前端
    ...
    """
    result = {"status": str(code), "message": msg, "data": "", "response": "", "next": next, "error": error}
    if str(code) == "200":
        result["response"] = "ok"
    else:
